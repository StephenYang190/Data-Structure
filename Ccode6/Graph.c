...
int node_matrix[7] = {15,18,1,5,9,6,3};
statue CreateGraph(GraphPoint g)
{
    ...
    for(int i =0;i < Node_num;i++)
    {
        ...
        if(scanf("%d",&num) == 1)
        {*/
            g->node[g->node_nowsite].data = node_matrix[i];
...
int LocateVex(Graph g,int elem)
{
    int i = 0;
...
int GetVex(Graph g,int site)
{
    if(site > g.node_nowsite)
        return -1;
    return g.node[site - 1].data;
}
statue PutVex(GraphPoint g,int site,int elem)
{
    if(site > g->node_nowsite)
        return false;
